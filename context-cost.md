...
`agent-workflow.md` is still the single largest file — over a fifth of the cost of a full load — after the v1.9 compression pass cut it ~16% (3,039 → 2,555 words) without dropping a rule. `writing-rules.md` had the same treatment at v1.5, ~14% (1,481 → 1,275 words), and `writing-patterns.md` at v1.1, ~9% (1,792 → 1,622 words) — none of the three dropped a rule.
...
| Everything at once | entry + all seven `ai-docs/` files | ~18,150 |

## Caveats

- This is a one-time context-window cost **per session**, paid when files are actually `Read` — not merely linked to. The graduated-loading rule exists specifically to avoid paying the ~18.2k full cost on every task.
