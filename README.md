...
**The same package installs the same way for everyone.** Every install lands three entry files — `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` — plus an `ai-governance/` rules directory, covering Claude Code, OpenAI Codex, the GitHub Copilot CLI and coding agent, Antigravity CLI, and any other tool that reads a root `AGENTS.md` directly, in one pass. **Which AI tool you use doesn't change *what* gets installed — only *how* you install it, and which entry file that tool then reads.**
...
| **Antigravity CLI** | `GEMINI.md` (which imports the same four, via the `@./` syntax its resolver documents) | **Path B** — hand your agent the procedure |
| **OpenAI Codex, the Copilot CLI / coding agent, or any other `AGENTS.md` reader** | `AGENTS.md` | **Path B** — hand your agent the procedure |
| **No agent in the loop** (you are copying files yourself) | *(all three — they land as files)* | **Path C** — copy the files by hand |
...
One honest caveat, whichever path you take: **Claude Code's `@import` reliably pulls `AGENTS.md` into context; every other supported CLI loads `AGENTS.md` directly but has no import mechanism, so it does not reliably pull the relative-linked `ai-governance/*.md` files in the same way.** Antigravity CLI sits between the two cases: it loads `AGENTS.md` directly the way Codex does — so it is never ungoverned — *and* it reads a root `GEMINI.md`, which is why `GEMINI.md` ships carrying the same imports. Whether the expansion actually fires inside a workspace-root `GEMINI.md` is undocumented, and has not been confirmed the way the Claude Code behavior has. (Behavior varies by tool; see `testing/coverage-matrix.md`.) That is why each entry file restates the non-negotiable core inline and tells the agent, in imperative terms, to open `core-rules.md` before any task (and `coding-rules.md` / `writing-rules.md` before code / content). The core binds regardless; the full rules bind reliably on Claude Code, are *intended* (not yet measured) to bind the same way on Antigravity CLI, and depend on the agent following the link elsewhere. Human review before merge (below) is the backstop.
...
| `ai-docs/GEMINI.template.md` | `GEMINI.md` *(repo root)* | Thin Antigravity CLI pointer: the same four imports, written `@./AGENTS.md`, `@./ai-governance/core-rules.md`, `@./ai-governance/agent-workflow.md`, `@./ai-governance/client-profiles.md` — the explicit `./` form Antigravity's resolver documents, which Claude Code's does not require. Unlike `CLAUDE.md`, this file is **not** what makes the repo governed for its tool: Antigravity reads root `AGENTS.md` too. It ships to turn two links into imports. Keep all four import lines, same reasoning as `CLAUDE.md`. |
...
Either way it shows you what changed before changing anything, then works in tiers — the six portable rule files and the thin `CLAUDE.md` and `GEMINI.md` are replaced outright (a target installed before 2026-08-21 gets `GEMINI.md` added fresh, since it predates the file), while `AGENTS.md` and `ai-governance/client-profiles.md` are **merged**, because those two carry local content an overwrite would destroy. `ai-governance/client-profiles/` is never touched at all.

**By hand** (no agent in the loop), the same split is what matters. Safe to re-copy straight from `ai-docs/`: `core-rules.md`, `coding-rules.md`, `writing-rules.md`, `coding-patterns.md`, `writing-patterns.md`, `agent-workflow.md`, and the thin `CLAUDE.md` and `GEMINI.md` (re-strip each one's banner). **Do not re-copy** `AGENTS.md` or `ai-governance/client-profiles.md`: bring the upstream changes into them by hand instead, keeping your filled placeholders, your `Active client` value — it appears **twice**, in the header *and* inside the mandatory-rules block — and your active-client list. Leave `ai-governance/client-profiles/` alone.
...
> **client profile > `core-rules.md` > `coding-rules.md` / `writing-rules.md` > `coding-patterns.md` / `writing-patterns.md` / `agent-workflow.md` > project entry file** *(`AGENTS.md` / `CLAUDE.md`)*
