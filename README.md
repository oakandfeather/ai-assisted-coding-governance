...
| `ai-docs/CLAUDE.template.md` | `CLAUDE.md` *(repo root)* | Thin Claude Code pointer: `@AGENTS.md`, plus `@ai-governance/core-rules.md` and `@ai-governance/agent-workflow.md`. Claude Code auto-loads `CLAUDE.md` from the root only — not a subfolder, not under the `.template` name — and imports the shared body. Keep all three import lines: `AGENTS.md` only *links* the rule files, and the two always-on ones are imported here so they cannot be skipped. |
