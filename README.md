...
**Scope: coding CLIs.** As of 2026-08-21 the package targets command-line coding agents only. In-IDE assistants that use their own instruction path — GitHub Copilot Chat and inline suggestions, which read `.github/copilot-instructions.md` — are **no longer supported**, and that file is no longer shipped. If you need Copilot in the IDE governed, that is a second entry file to maintain by hand; see the trade-off recorded in [`AGENTS.md`](./AGENTS.md) under *Multi-tool entry points*.
...
One honest caveat, whichever path you take: **Claude Code's `@import` reliably pulls `AGENTS.md` into context; every other supported CLI loads `AGENTS.md` but has no import mechanism, so it does not reliably pull the relative-linked `ai-governance/*.md` files in the same way.** (Measured once against Codex on 2026-08-21, that expectation came out backwards — Codex *did* open the linked files, 4/4. One draw is not enough to revise the caveat; see `testing/coverage-matrix.md`.) That is why each entry file restates the non-negotiable core inline and tells the agent, in imperative terms, to open `core-rules.md` before any task (and `coding-rules.md` / `writing-rules.md` before code / content). The core binds regardless; the full rules bind reliably on Claude Code and depend on the agent following the link elsewhere. Human review before merge (below) is the backstop.
