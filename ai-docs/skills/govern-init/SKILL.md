...
Scaffolds the governance package into a target project. This skill **copies** the rules; it does not contain them. The rule text lives in the copied files, where it auto-loads every session and stays auditable in the client's repo.

## Source package

The package lives in the governance repo (`ai-docs/`). Locate it in this order:
...
Do not copy `human-docs/` — it is for people and stays in the governance repo.
...
- **`AGENTS.md`** — retitle `# AGENTS.md (template)` to `# AGENTS.md` and delete the banner block above the `Owner:` line. It describes the template rather than the project, and its `../CLAUDE.md` link does not resolve outside the governance repo. This is the file whose placeholders you fill (step 5).
