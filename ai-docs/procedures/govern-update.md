...
**The one addition you will actually meet: `GEMINI.md`.** Any target installed before 2026-08-21 has no `GEMINI.md` at all — it predates Gemini CLI support. This is not an `ai-governance/` file, so the generic "added upstream" bullet above does not apply as written: scaffold it the same way `govern-init` does, by taking `GEMINI.template.md` from the source, stripping its banner per that procedure's step 3, and writing the result to the target's root as a new file (there is nothing to diff against — the target has none). Treat it as tier B going forward. Ask before adding it, the same as any other new file this procedure would introduce.
