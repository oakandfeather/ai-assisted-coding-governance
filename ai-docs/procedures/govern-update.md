...
| **B — banner-stripped** | `CLAUDE.md`, `GEMINI.md` | Re-derive from the template (slice the banner per step 2), then replace — but diff first, and gate it separately. See below. |
...
**Tier B carries no placeholders, but that is not what makes a file safe to replace.** The risk to `CLAUDE.md` and `GEMINI.md` is **appended local content**: each is the natural place for a team to add tool-specific project notes beneath its `@AGENTS.md` import, and `govern-init` step 1 already refuses to overwrite a pre-existing entry file for exactly that reason. So give tier B **its own diff and its own gate** rather than folding it into the tier-A batch — one gate can cover both files, since neither carries placeholders and the same logic applies to each. If the target holds anything the incoming template does not, treat it as local customization: surface it and ask. **One exception, in the other direction:** `CLAUDE.md`'s `@ai-governance/core-rules.md`, `@ai-governance/agent-workflow.md`, and `@ai-governance/client-profiles.md` imports — and `GEMINI.md`'s identical three, written with the `./` prefix its resolver requires — are *shipped* routing, not local content — they are in the incoming template. If the target is missing any of them, that is an install predating them or a well-meant "keep it thin" trim, and the replacement restores them; say so rather than flagging it as a loss. **The exception covers the whole routing block, not just the `@` lines** — the lead-in sentence above the imports and the maintainer HTML comment above that are shipped text too. A target whose wording differs there is running an *older shipped version* of the block, not a team's customization, so take the incoming template's and report it as a routing refresh. Expect this: the 2026-08-20 index import reworded both, so every `CLAUDE.md` install predating it surfaces two changed lines on its next update; a target install that predates `GEMINI.md` entirely (2026-08-21) will not have the file at all — treat that as **added upstream** (see below), not as a diff. Text **outside** that block — anything appended below the imports — is local content, and the ask-first rule above still governs it. Silently dropping a team's notes inside a batch nobody reads is the worst of both.
...
**The one addition you will actually meet: `GEMINI.md`.** Any target installed before 2026-08-21 has no `GEMINI.md` at all — it predates Antigravity CLI support. This is not an `ai-governance/` file, so the generic "added upstream" bullet above does not apply as written: scaffold it the same way `govern-init` does, by taking `GEMINI.template.md` from the source, stripping its banner per that procedure's step 3, and writing the result to the target's root as a new file (there is nothing to diff against — the target has none). Treat it as tier B going forward. Ask before adding it, the same as any other new file this procedure would introduce.
