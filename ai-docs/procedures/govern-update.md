...
**Existing profile and policy filenames stay as they are.** The `<client>-profile.md` / `<client>-policy.md` convention in the refreshed `client-profiles.md` governs files authored **from now on**. An install predating it holds names like `esu.md`, so the refreshed index will describe a shape those files do not match - that is cosmetic, because the index links each profile by its own path and nothing dangles. Tier E still governs: **do not rename them**, and do not report the mismatch as a defect. Name the convention once in the tier-D hand-off so the team can rename on their own schedule if they want to.

**A target with profiles but no `client-policies/` is open work you must report.** `client-policies/` is created by `govern-init` step 6, not copied from source, so the "added upstream" handling below will never produce it — and every repo installed before it existed has none. Such a repo now carries a `client-profiles.md` paragraph pointing at `client-policies/<client>-policy.md` with no such directory: a pointer that resolves to nothing, in the file whose job is resolving the override chain. So check for it, and if `ai-governance/client-profiles/*.md` exists while `ai-governance/client-policies/` does not, say so in the tier-D hand-off as open work — supply the policy, or convert the profile's authority note to the cite-only shape (title, version, canonical URL). **Do not create the directory and do not write a policy into it.** Reporting is the whole job here; silence is the one unacceptable outcome.
...
**Tier B carries no placeholders, but that is not what makes a file safe to replace.** The risk to `CLAUDE.md` is **appended local content**: it is the natural place for a team to add Claude-specific project notes beneath the `@AGENTS.md` import, and `govern-init` step 1 already refuses to overwrite a pre-existing `CLAUDE.md` for exactly that reason. So give tier B **its own diff and its own gate** rather than folding it into the tier-A batch. If the target holds anything the incoming template does not, treat it as local customization: surface it and ask. **One exception, in the other direction:** `CLAUDE.md`'s `@ai-governance/core-rules.md`, `@ai-governance/agent-workflow.md`, and `@ai-governance/client-profiles.md` imports are *shipped* routing, not local content — they are in the incoming template. If the target is missing any of them, that is an install predating them or a well-meant "keep it thin" trim, and the replacement restores them; say so rather than flagging it as a loss. **The exception covers the whole routing block, not just the `@` lines** — the lead-in sentence above the imports and the maintainer HTML comment above that are shipped text too. A target whose wording differs there is running an *older shipped version* of the block, not a team's customization, so take the incoming template's and report it as a routing refresh. Expect this: the 2026-08-20 index import reworded both, so every install predating it surfaces two changed lines on its next update. Text **outside** that block — anything appended below the imports — is local content, and the ask-first rule above still governs it. Silently dropping a team's notes inside a batch nobody reads is the worst of both.
