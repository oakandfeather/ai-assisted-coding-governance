...
*The task-agnostic base rules for any AI agent working on a client engagement — whether it writes code, edits documentation, does research, or drafts and analyzes content. Reference it from your project's entry file — `AGENTS.md`, `CLAUDE.md`, or `.github/copilot-instructions.md` (e.g., `See core-rules.md for the mandatory rules that apply to every task`). These rules exist to mitigate the risks of AI-generated work. Follow them on every task. When a client profile (see [`client-profiles.md`](./client-profiles.md)) is stricter, the client profile wins. Two task modules add to this base *(each installed per repository — see the note below)*: [`coding-rules.md`](./coding-rules.md) (code-specific) and [`writing-rules.md`](./writing-rules.md) (documentation/content-specific) — open whichever your task calls for and this repo has. Companion: [`agent-workflow.md`](./agent-workflow.md) (how to work).*

**Version:** 1.2 · **Last reviewed:** 2026-08-06 · **Review cycle:** Quarterly, or whenever a client's AI terms change.

**This package installs in modules.** `core-rules.md` and `client-profiles.md` are always present; the task modules and craft companions are chosen per repository at install time. So if a file named, linked, or cited by section number anywhere in this package is not in `ai-governance/`, that module was not installed here — the pointer simply does not apply, and nothing is missing or broken. **Do not reconstruct an absent module's rules from memory**; a paraphrased rule is not the rule. If a task plainly needs a module this repo doesn't have, say so and ask for it to be installed.
