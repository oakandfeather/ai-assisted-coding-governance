...
>
> **How to use it.** This is the **canonical entry file** — the one full body every agent reads. Copy it to the target project's **repository root** as `AGENTS.md` (Codex CLI, the GitHub Copilot CLI and coding agent, and every other `AGENTS.md` reader load `AGENTS.md` from the root). Put `core-rules.md`, `coding-rules.md`, `writing-rules.md`, `database-rules.md`, `coding-patterns.md`, `writing-patterns.md`, `agent-workflow.md`, `client-profiles.md`, and the `client-profiles/` directory together in an **`ai-governance/` directory at the repo root**, so the relative `./ai-governance/` links below resolve. Then fill in the italicized placeholders. **Unfilled placeholders mean the repo is unconfigured: ask before assuming a stack, client, or command — do not invent them.**
>
> **One thin companion travels with it**, because Claude Code is the one supported CLI that does not read `AGENTS.md` directly: `CLAUDE.template.md` → `CLAUDE.md` at the repo root (Claude Code reads it and `@`-imports this file, plus `core-rules.md`, `agent-workflow.md`, and the `client-profiles.md` index — the two rule files that bind on every task and the pointer to the client overrides that outrank them, all of which this file only links). Every other supported CLI reads this root `AGENTS.md` itself. Fill the placeholders **here** — the companion carries none.
>
...
- **[`database-rules.md`](./ai-governance/database-rules.md) — working in a database project**, where the schema itself is the deliverable. Deploy guards, destructive schema change, reading the generated deploy script, drift, and the data that ships with the schema.
...
---
*Fill in the italicized placeholders for this repository. This file lives at the repo root as `AGENTS.md`; `core-rules.md`, `coding-rules.md`, `writing-rules.md`, `database-rules.md`, `coding-patterns.md`, `writing-patterns.md`, `agent-workflow.md`, `client-profiles.md`, and the `client-profiles/` directory belong together in the `ai-governance/` directory beside it (see the note at the top). The thin `CLAUDE.md` companion points back here — keep the placeholders filled in this file. `CLAUDE.md` additionally `@`-imports `core-rules.md`, `agent-workflow.md`, and the `client-profiles.md` index, because a linked file the agent never opened never loaded; that is routing, not local customization, so keep those import lines.*
