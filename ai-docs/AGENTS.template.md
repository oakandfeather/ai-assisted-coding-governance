...
**Version:** 1.14 · **Last reviewed:** *(date)* · **Active client:** *(client name)*
...
---
*Fill in the italicized placeholders for this repository. This file lives at the repo root as `AGENTS.md`; `core-rules.md`, `coding-rules.md`, `writing-rules.md`, `coding-patterns.md`, `writing-patterns.md`, `agent-workflow.md`, `client-profiles.md`, and the `client-profiles/` directory belong together in the `ai-governance/` directory beside it (see the note at the top). The thin `CLAUDE.md` companion points back here — keep the placeholders filled in this file. `CLAUDE.md` additionally `@`-imports `core-rules.md`, `agent-workflow.md`, and the `client-profiles.md` index, because a linked file the agent never opened never loaded; that is routing, not local customization, so keep those import lines.*
