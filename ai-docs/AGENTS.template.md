...
>
> **How to use it.** This is the **canonical entry file** — the one full body every agent reads. Copy it to the target project's **repository root** as `AGENTS.md` (Codex CLI, the GitHub Copilot CLI and coding agent, and every other `AGENTS.md` reader load `AGENTS.md` from the root). Put `core-rules.md`, `coding-rules.md`, `writing-rules.md`, `coding-patterns.md`, `writing-patterns.md`, `agent-workflow.md`, `client-profiles.md`, and the `client-profiles/` directory together in an **`ai-governance/` directory at the repo root**, so the relative `./ai-governance/` links below resolve. Then fill in the italicized placeholders. **Unfilled placeholders mean the repo is unconfigured: ask before assuming a stack, client, or command — do not invent them.**
>
> **Two thin companions travel with it**, both at the repo root: `CLAUDE.template.md` → `CLAUDE.md` and `GEMINI.template.md` → `GEMINI.md`. Each reads its own file and `@`-imports this one, plus `core-rules.md`, `agent-workflow.md`, and the `client-profiles.md` index — the two rule files that bind on every task and the pointer to the client overrides that outrank them, all of which this file only links. `CLAUDE.md` is **required**: Claude Code reads no `AGENTS.md` at all. `GEMINI.md` is the import vehicle for Antigravity CLI, which *does* read this root `AGENTS.md` — so it adds delivery, not coverage. Every other supported CLI reads this root `AGENTS.md` itself. Fill the placeholders **here** — neither companion carries any.
>
...
---
*Fill in the italicized placeholders for this repository. This file lives at the repo root as `AGENTS.md`; `core-rules.md`, `coding-rules.md`, `writing-rules.md`, `coding-patterns.md`, `writing-patterns.md`, `agent-workflow.md`, `client-profiles.md`, and the `client-profiles/` directory belong together in the `ai-governance/` directory beside it (see the note at the top). The thin `CLAUDE.md` and `GEMINI.md` companions point back here — keep the placeholders filled in this file. Each additionally `@`-imports `core-rules.md`, `agent-workflow.md`, and the `client-profiles.md` index (`GEMINI.md` with the `./` prefix its resolver documents), because a linked file the agent never opened never loaded; that is routing, not local customization, so keep those import lines.*
