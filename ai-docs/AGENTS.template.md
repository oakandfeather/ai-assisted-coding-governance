...
>
> **Two thin companions travel with it**, so the other tools load this same body: `CLAUDE.template.md` → `CLAUDE.md` at the repo root (Claude Code reads it and `@`-imports this file, plus `core-rules.md`, `agent-workflow.md`, and the `client-profiles.md` index — the two rule files that bind on every task and the pointer to the client overrides that outrank them, all of which this file only links), and `copilot-instructions.template.md` → `.github/copilot-instructions.md` (Copilot's repository-wide custom-instructions path for Chat, inline suggestions, and supported features). Copilot agent workflows can also use this root `AGENTS.md`. Fill the placeholders **here** — the companions carry none.
>
...
**Version:** 1.13 · **Last reviewed:** *(date)* · **Active client:** *(client name)*
