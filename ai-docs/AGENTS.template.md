...
>
> **How to use it.** This is the **canonical entry file** — the one full body that every agent reads. Copy it to the **repository root** of the target project and rename it to `AGENTS.md` (Codex, the GitHub Copilot coding agent, Cursor, Windsurf, and VS Code's agent all read `AGENTS.md` from the root). Put `core-rules.md`, `client-profiles.md`, the `client-profiles/` directory, and whichever of the optional modules — `coding-rules.md`, `coding-patterns.md`, `writing-rules.md`, `writing-patterns.md`, `agent-workflow.md` — this project selected at install time together in an **`ai-governance/` directory at the repo root**, so the relative `./ai-governance/` links below resolve. **Then delete the bullet line for every module you did not install**; if you installed none of them, delete the whole list together with its lead-in paragraph and the "Where craft meets safety" line that follows it, so nothing points at a file that isn't there. Then fill in the italicized placeholders below. **If the placeholders are still unfilled, treat the repo as unconfigured: ask before assuming a stack, client, or command — do not invent them.**
>
...
Then read the module your task calls for. **Load it in one pass before you start reading this project's code or content — not after you have picked an approach**: a rule reached after the work is shaped is too late to shape it.

- **[`coding-rules.md`](./ai-governance/coding-rules.md)** — before writing, editing, or running code (dependencies, security, testing, accessibility).
  - **[`coding-patterns.md`](./ai-governance/coding-patterns.md)** — the craft companion; read before writing or editing non-trivial code.
- **[`writing-rules.md`](./ai-governance/writing-rules.md)** — before producing or editing documents and content, including documentation about code such as READMEs, API references, and runbooks (factual grounding, citations, confidentiality, voice fidelity, accessible documents, verified documentation).
  - **[`writing-patterns.md`](./ai-governance/writing-patterns.md)** — the craft companion; read before writing or editing a non-trivial document (audience, structure, economy, and what a README, runbook, or API reference owes its reader).
- **[`agent-workflow.md`](./ai-governance/agent-workflow.md)** — how to work: the work loop, the ask-vs-proceed-vs-object boundary, verification, structured hand-off, bounded iteration and adversarial self-review, economy of effort, subagent delegation.
...
---
*Fill in the italicized placeholders for this repository. This file lives at the repo root as `AGENTS.md`; `core-rules.md`, `client-profiles.md`, the `client-profiles/` directory, and the optional modules this project selected at install time belong together in the `ai-governance/` directory beside it (see the note at the top). The thin `CLAUDE.md` and `.github/copilot-instructions.md` companions point back here — keep the placeholders filled in this file.*
