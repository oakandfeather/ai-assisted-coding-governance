...
>
> **Two thin companions travel with it**, one for each supported CLI that does not read `AGENTS.md` directly: `CLAUDE.template.md` → `CLAUDE.md` and `GEMINI.template.md` → `GEMINI.md`, both at the repo root (Claude Code and Gemini CLI each read their own file and `@`-import this one, plus `core-rules.md`, `agent-workflow.md`, and the `client-profiles.md` index — the two rule files that bind on every task and the pointer to the client overrides that outrank them, all of which this file only links). Every other supported CLI reads this root `AGENTS.md` itself. Fill the placeholders **here** — neither companion carries any.
>
